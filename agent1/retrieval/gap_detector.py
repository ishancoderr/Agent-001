...
Execute the local SQL lookup and classify every (state, year) outcome as:
  ...
  - temporal_gap  : no row at all for a year in a state Agent-1 does have
  - spatial_gap   : state has zero rows in Agent-1's state_demographics at all
"""
...
def _print_sql(label: str, sql: str, params: Dict) -> None:
    """Print a SQL query cleanly to the terminal so it can be copied."""
    line = "=" * 60
...
WHERE s.state_name = ANY(ARRAY[{states_arr}]);"""
        ...
        _print_sql("STEP A — Partition check (which states exist in Agent-1)", sql_partition, {
            "states": states,
        })
        ...
        log.info("       │ States found in Agent-1 partition : %s", sorted(states_in_db))
        ...
        if spatial_gap_states:
            log.info("       │ SPATIAL GAP (not in partition)    : %s → will ask Agent-2", spatial_gap_states)
        if not local_states:
...
def execute_local_lookup_from_slots(slot) -> Dict[str, Any]:
    """Used when Agent 2 sends a KQML ask TO Agent 1 (bidirectional)."""
...
WHERE s.state_name = ANY(ARRAY[{states_arr}])
  AND sd.stat_year  = ANY(ARRAY[{years_arr}]);"""
    ...
    _print_sql("Bidirectional lookup (Agent-2 → Agent-1)", sql, {
        "states": states,
        "years":  years,
    })
    ...
    try:
        ...
        for row in rows:
            ...
            if all(v is not None for v in vals.values()):
                fully_present_keys.add((state, yr))

        missing_states = [
            s for s in states
            if any((s, yr) not in fully_present_keys for yr in years)
        ]
