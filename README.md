# Multi-Agent Geospatial Missing Data System — Agent 1

A KQML-based multi-agent system for handling missing data in a geospatial statistics
dataset. Two autonomous agents (this repo, and its sibling **Agent-002**) each hold their
...
QueryResponse
  ...
  .provenance = {kqml_turns, records_from_agent_1, records_from_agent_2, ...}
```
...
└── agent1/
    ...
    ├── .env                         ← DB credentials, AGENT2_URL, OPENAI_API_KEY, model overrides
    │
    ├── controller/
...
Agent-1                                           Agent-2
   │  POST /kqml/receive                             │
   │ ─────────────────────────────────────────────► │
   │  {"performative": "ask", "sender": "Agent-1",   │
   │   "receiver": "Agent-2", "reply_with": "req-1", │
...
### 2. Configure environment (`agent1/.env`)
